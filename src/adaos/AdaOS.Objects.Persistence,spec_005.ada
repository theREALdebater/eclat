...
package AdaOS.Objects.Persistence is
   ...
   --| A class identifier is a string which contains the fully qualified name of the base type of an Ada class.
   ...
   --| A class set is a set of class identifiers. The Iterate procedure iterates over the members
   of a class set. The Is_In function returns true if a given identifier is a member of a class set.

   function Object_Classes (Container: in Object_Container;
                            OID:       in Object_Identifier) return Class_Set is abstract;

   --| The Object_Classes function returns the set of Ada classes that the object identified by the given OID
   will belong to (when engaged). This will include the name of every class that a class is derived from (but
   no name will be repeated). In other words, the set of every T'Class_Name for every type T for which
   "X in T'Class" returns True, where X is the object engaged by the given OID.
   ...
   --| The function Is_Link returns True is the OID designates an object link, and False otherwise.
   This function is a convenient alternative to Is_In(Container.Classes(OID),"Object_Link"). It may
   ...
   --| For an OID which designates an object link, the subprograms
   Object_Description, Object_Status, Object_Classes,
   Is_Engaged, and Engage all operate on the target object of the link, not on the link itself.
   The subprograms Link_Description, Link_Status, Link_Is_Engaged, and Engage_Link provide the
   same operation respective on the link object itself. If the OID used for any of these subprograms
   does not designate an object link, Status_Error is propagated.
