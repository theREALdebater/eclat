...
package AdaOS.IP_DNS is
   ...
   type Resource_Record (RRT: Resource_Record_Type) is
      record
         case RRT is
            when Zero_Marker | ..... =>
               null record;
   ...
   --| Both overloadings of the Resolve function perform a recursive lookup of the
   given domain name. The first expects to find an A record (potentially following
   a chain of CNAME records first), returning an IPv4 node address. The second
   expects to find a PTR record, returning its payload (as a string).
