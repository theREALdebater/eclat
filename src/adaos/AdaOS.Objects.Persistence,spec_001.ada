...
--| Activate(O,S) must never have any effect on O (providing no exception is raised by either
--| call). The behaviour of a persistent object must depend only on its state: any operation on
--| object O1 must have the same effect as the same operation (with the same input parameter
--| values) on an object O2 if O1 and O2 are of the same type and have the same state.
