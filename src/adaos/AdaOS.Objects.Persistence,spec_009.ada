...
package AdaOS.Objects.Persistence is
   ...
   --| add information as to its being shelved and other information related to its persistence.

   --\
   --/ Type of a member:

   function Object_Type (Container: in Object_Container;
                         OID:       in Object_Identifier) return Ada.Tags.Tag is abstract;

   --| The Object_Type function returns tag of the (tagged) Ada type of the member identified by
   --| the given OID (when it is engaged and a reference to it obtained).
   ...
   function OID (Container: in Object_Container;
                 Token:     in Engagement_Token) return Object_Identifier is abstract;
   ...
   --| and passes out, in the parameter Token, an engagement
   --| token.

   --| A member can be engaged any number of times without any specific limitation. Each
   time, a different token will be passed out.

   procedure Disengage (Container: in  Object_Container;
                        Token:     in  Engagement_Token) is abstract;

   --| The Disengage procedure disengages the given
   --| Token. The member persistent object is disengaged only when there are no
   remaining engaged tokens.
   ...
   --| Status_Error is propagated. If the object is shelved when this function is called,
   it is unshelved (so as to create an in-memory object to be referenced).
