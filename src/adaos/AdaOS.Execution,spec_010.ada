...
Replace it with:
   ...
   --| The fact that a process cannot execute these procedures to modify itself means that, in
   --| order to have the (very typical) functionality of changing the current directory during
   --| execution, a process must keep its own copies of the access values pointing to the
   --| current directories (initialised by the System_Files, Cluster_Files, and Unit_Files
   --| functions), and use (and change) these copies instead.

   --| Throughout AdaOS, the default current directory for each tree is the root of the tree.
