...
with AdaOS.Objects;
with AdaOS.Objects.Text;

use AdaOS.Execution;
use AdaOS.Objects;
use AdaOS.Objects.Text;

package body Ada.Directories 
is
...
--\
   ...
   is
      Env: Directory_Access;
      CWD: Object_Access;
   
   begin
      Env := Self.Compartment.Find("env");
      if Env = null then return ""; end if;

      select
         delay 1.0; -- maximum wait of 1 second
         return "";
      then abort
         Env.Engage;
         CWD := Env.Find("CWD");
         Env.Disengage;
         if CWD = null then return ""; end if;
         if not CWD.all in Text_File'Class then return ""; end if;
         CWD.Engage;
         declare
            Text: Text_File'Class renames Text_File'Class(CWD.all);
            Result: constant String := Text.Get_All;
         begin
            CWD.Disengage;
            return Result;
         end;
   ...
   exception
      when others =>
         if Env /= null and Env.Is_Engaged then Env.Disengage; end if;
         if CWD /= null and CWD.Is_Engaged then CWD.Disengage; end if;
         raise;
