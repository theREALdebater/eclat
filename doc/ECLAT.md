...
files (I intend to allow object files to be linked in, but this would have certain 
consequences). However, this shouldn't prevent existing software being readily ported. 
...
A wrapped library is kept as a set of files within one directory. When a (re)compilation 
occurs, a new file is added, which might contain library units that supersede those in 
previously generated files. There are also files that keep an index to the source files, ASIS 
data, and so on. 
...
Conceptually, ECLAT will build 'modules' from libraries. Each module is a pseudo-code (PXC) 
file (with the file extension `.pxc`) and gets 'realised' into native machine code. 

Generally, modules have a static set of dependency relationships with one another. The realizor 
will, by default, generate a primary ('main') executable (file) by gathering the downward 
closure of all the modules it depends on, directly or indirectly, and translating them all 
together as a whole. 

However, the realizor will allow, via configuration, for a module, together with all the 
modules it depends on, to be translated into an auxiliary executable file. Existing platforms 
(operating systems) have their own mechanisms for shared code: on Microsoft(R) Windows(R), it 
is Dynamic Link Loadable (DLL) files; on Linux/elf it is `.so' (shared object) files; there are 
similar facilities on other platforms. Whenever the realizor generates an executable file which 
depends on a module that has been realised into an auxiliary executable file, it makes calls 
into the auxiliary executable file (and arranges for it to be loaded). 

ECLAT will have a mechanism, using aspects, to enable a package -- officially termed an 
'extension module' but colloquially called a 'plug-in' -- to be built into an optional module 
that other modules can depend on, but can do without. ECLAT ensures that the plug-in package 
can be omitted without fundamentally breaking everything. 

When the realizor gathers modules to realise an executable file, it gathers as many optional 
modules as it can (searching in configurable directories, for example) and includes whichever 
ones it can find. It is possible for an optional module to also be an auxiliary executable 
file, but it doesn't have to be. 
...
* use program arguments (interpreting options etc).
...
The framework will define a security model. That is a whole big story in itself.
